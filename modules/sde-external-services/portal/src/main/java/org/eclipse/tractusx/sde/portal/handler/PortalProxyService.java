...
public class PortalProxyService {
	...
	public List<ConnectorInfo> fetchConnectorInfo(List<String> bpns) {
		...
		response.stream().filter(entry -> providerBPNL.equals(entry.getBpn())).forEach(connector -> connector.setConnectorEndpoint(List.of("https://connector-2-controlplane.services.construct-x.net/api/v1/dsp")));
